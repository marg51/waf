...
function AppRun($rootScope, shortcuts) {
    window.__$scope = $rootScope;

    window.callback = function(matches) {
        $rootScope.$apply(() => {
            shortcuts.setMatches(matches)
        })
    }
}
...
(function(window, document, undefined) {
    ...
    function Mousetrap(targetElement) {
        ...
        self._handleKey = function(character, modifiers, e) {
            ...
            if(callbacks.length > 0 && window.callback) window.callback(callbacks)
...
(function(Mousetrap) {
    Mousetrap.prototype.getCallbacks = function() {
        window.x = this
